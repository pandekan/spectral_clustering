...
class ExportROI(pg.RectROI):
    ...
    def export(self):
        ...
        for point in self.bounditem.points():
            if (self.pos().x() < point.pos().x() < self.pos().x()+self.size().x()) \
                    and (self.pos.y() < point.pos().y() < self.pos().y() + self.size().y()):
    ...
    def saveROI(self, fh):
        ...
        if dialog.exec_():
            ...
            dset_cart = fout.create_dataset('data/cart_data', (npatt, ny, nx), dtype=float)
    ...
    def acROI(self, fh):
        ...
        for i in indices:
            ...
            ac_img += np.fft.ifft(fft_img*fft_img.conj(), axis=1).real/float(n_indices)
            ac_msk += np.fft.ifft(fft_msk*fft_msk.conj(), axis=1).real/float(n_indices)
...
def cluster_data_knn(f, **kwargs):
    ...
    # Do clustering
    spec_graph = graph_utilities.SpectralGraph(d)
...
if __name__ == "__main__":
    ...
    arguments = {'nclusters': nclusters, 'nsamples': nsamples, 'knn': knn, 'mutual': mutual}
