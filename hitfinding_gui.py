...
def cluster_data(f, **kwargs):
    ...
    if 'start_ind' in keys:
        start_ind = kwargs['start_ind']
    else:
        start_ind = 0

    select_ind = np.arange(start_ind, start_ind + nsamples)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-roi', '--roi', help='x1,y1,x2,y2 for region of interest',
                        default='300,300,724,724')  # ,type=int)
    parser.add_argument('-start', '--start', help='starting index for data', default=0, type=int)
    ...
    if start_ind > d.shape[0]:
        start_ind = 0

    if (start_ind + nsamples) > d.shape[0]:
        nsamples = d.shape[0] - start_ind
    ...
    # cluster the data
    arguments = {'nclusters': nclusters, 'nsamples': nsamples, 'start_ind': start_ind,
                 'knn': knn, 'mutual': mutual, 'metric': metric}
